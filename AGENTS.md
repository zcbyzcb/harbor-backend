...
| harbor-api | Controller、HTTP Request/Response、接口组装、全局 HTTP 异常转换和认证。 | app、领域公开类型及入口框架。 |
| harbor-app | Processor、Qurier、Command、Query、用例 DTO、应用服务和业务 Job。 | domain；查询侧可依赖 infrastructure 的只读 Mapper。 |
...
- Handler 放在 harbor-app 对应业务包的 `job/` 下，使用 `@XxlJob("syncFutureInventoryJob")` 注册唯一名称。
